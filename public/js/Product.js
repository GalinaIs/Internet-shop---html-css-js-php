 ...
 */
var Product = function (myCart, idBrowseAll, idElementHide = '', idElementHtml = '', sortBy = '') {
...
Product.prototype.renderOnePage = function (dataAfterSearch, count, numberPage) {
    ...
    for (var ind = (numberPage - 1) * count; ind < numberPage * count; ind++) {
        ...
        $('.template .ref_prod').attr('href', '/product/card?id=' + dataAfterSearch[ind].id);
...
Product.prototype.renderProduct = function (countForPrint, numberPage = 1, flagAll = false) {
    $.ajax({
        ...
        success: function (data) {
            console.log(data);
            var dataAfterSearch = this.getItemForPrint(data.products);
...
Product.prototype.addToCartDialog = function (event) {
    ...
    var self = this;
    $.ajax({
        url: '/product/getOneProduct?id=' + idProduct,
        dataType: 'json',
        context: this,
        success: function (data) {
            ...
            $dialogCart.append($('<img />', {
                src: this.pathHtml + data.product.path_img
            }));
            ...
            this.createSelect('dialog_size_item', data.product.size);
            ...
            this.createSelect('dialog_color_item', data.product.color);
...
Product.prototype.addToCart = function (idProduct, sizeItem, colorItem, quantityItem = 1, isValidAdd = true,
    ...
    if (isValidAdd) {
        ...
        $.ajax({
            url: '/basket/addProduct?id=' + idProduct + '&count=' + quantityItem + '&size=' + 
            sizeItem + '&color=' + colorItem,
            context: this,
            dataType: 'json',
