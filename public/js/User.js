...
User.prototype.renderViewUser = function () {
    if (this.userCurrent === null || this.userCurrent.id === null || 
        this.userCurrent === undefined || this.userCurrent.id === undefined) {
        $('#user_noreg').show();
        $('#user_reg').hide();
    } else {
        $('#user_noreg').hide();
            ...
            click: function (event) {
                if ($('#dialog_forgot_password input')[0].validationMessage === '') {
                    $.ajax({
                        ...
                        success: function (data) {
                            for (var ind = 0; ind < data.users.length; ind++) {
                                if (data.users[ind].email === $('#dialog_forgot_password input').val()) {
                                    break;
                                }
                            }
                            if (ind === data.users.length) {
...
User.prototype.refreshUserData = function ($element) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        context: this,
        data: {json: JSON.stringify(refreshDataUser)},
        success: function () {
...
/**
 * Метод класса, который отображает диалог при входе в аккаунт пользователя - хочет ли пользователь продолжить работу с 
 * текущей корзиной. Если нет - загружаем корзину из базы.
 */
User.prototype.dialogLogIn = function () {
    $dialogLogIn = $('#dialog_registration');
    $dialogLogIn.html('Do you want shopping with current cart?');
    var self = this;
    ...
    $dialogLogIn.dialog({
        ...
        buttons: [{
            text: 'Yes',
            click: function () {
                self.cart.changeCartUserProfile(self.userCurrent.id);
                $(this).dialog('close');
                self.renderViewUser();
                if (self.accountHtml) {
...
User.prototype.logIn = function (email, password) {
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var ind = 0; ind < data.users.length; ind++) {
                if (data.users[ind].email === email && data.users[ind].password === password) {
                    break;
                }
            }

            if (ind !== data.users.length) {
                this.userCurrent = data.users[ind];
                ...
                $.ajax({
                    ...
                    context: this,
                    dataType: 'JSON',
                    data: {json: JSON.stringify(data.users[ind].id)},

                    success: function (data) {
                        if (data.flagChooseBasket) {
                            this.dialogLogIn();
                        } else {
                            this.renderViewUser();
                            if (this.accountHtml) {
                        ...
                        if (this.cart.isCartTable) {
                            this.cart.renderTableCart();
                        }
                    }
                });
            } else {
...
User.prototype.dialogLogOut = function (idUser) {
    ...
    $dialogLogOut.html('You exit from your account');
    var self = this;
...
User.prototype.logOut = function () {
    $.ajax({
        url: '/user/logOut?id=' + this.userCurrent.id,
        context: this,
        dataType: 'JSON',
        success: function () {
            this.userCurrent = {};
    ...
    $.ajax({
        ...
        success: function (dataUser) {
            $.ajax({
                ...
                context: this,
                dataType: 'JSON',
                data: {json: JSON.stringify(dataUser.id)},
            
                success: function () {
                    newUser.id = dataUser.id;
